...
public class ItemRequestToJsonTest extends BaseDtoJsonTest {
    ...
    void shouldSerializeItemRequestDto() throws Exception {
        ...
        assertThat(json).extractingJsonPathNumberValue("$.id")
                .isEqualTo(REQUEST_ID.intValue());

        assertThat(json).extractingJsonPathStringValue("$.description")
                .isEqualTo("test");

        assertThat(json).extractingJsonPathNumberValue("$.requesterId")
                .isEqualTo(USER_ID.intValue());

        assertThat(json).extractingJsonPathStringValue("$.created")
                .isEqualTo("2025-05-19T05:00:00");

        assertThat(json).extractingJsonPathNumberValue("$.owner.id")
                .isEqualTo(USER_ID.intValue());

        assertThat(json).extractingJsonPathArrayValue("$.items")
                .isEmpty();
    }
