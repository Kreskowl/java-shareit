...
public class FutureBookingByItemHandler implements BookingByItemHandler {
    ...
    public List<Booking> handle(List<Long> itemIds, BookingState state, LocalDateTime now) {
        return repository.findAllByItemIdInAndStartTimeAfterOrderByStartTimeDesc(itemIds, now);
    }
}
